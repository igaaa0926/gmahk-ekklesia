...
function renderPetugas(val: any): string {
  if (!val) return '─';
  if (typeof val === 'object') {
    // Jika data berupa objek hasil join relasi jemaat (misal { nama: 'Alex' })
    return val.nama || val.name || JSON.stringify(val);
  }
  return String(val);
}
...
function RoleRow({ label, value }: { label: string; value: any }) {
  return (
    <div className="flex items-start justify-between gap-4 py-2.5 border-b border-slate-100 last:border-0 text-sm">
      ...
      <span className="text-slate-800 font-semibold text-right">{renderPetugas(value)}</span>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 font-sans antialiased pb-16">
      ...
      <footer className="max-w-5xl mx-auto px-4 mt-12">
        <div className="pt-4 border-t border-slate-200 text-center">
          <a href="/admin" className="text-xs font-semibold text-slate-400 hover:text-slate-600 bg-slate-50 hover:bg-slate-100 px-4 py-2 rounded-full border border-slate-200/60 transition-all">
